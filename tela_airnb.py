...
def renderizar():
    ...
    ano_selecionado = utils.ano_atual
    ...
    aba_atual, aba_antigos = st.tabs(["🟢 Mês Atual & Anterior", "🕰️ Meses Antigos"])

    with aba_atual:
        st.caption(f"Foco total! Você está gerenciando: **{' e '.join(meses_atuais)}**.")
    ...
    with aba_antigos:
        if not meses_antigos:
            st.info("Não há meses antigos registrados neste ano ainda.")
        else:
            st.caption(f"Histórico: **{', '.join(meses_antigos)}**.")
            cols_ant = ['MESES'] + meses_antigos
            cfg_ant = get_col_config(meses_antigos)

            st.markdown("#### ⬆️ Entradas Antigas")
            ...
            for i, m in enumerate(meses_antigos):
                ...
                cols_res_ant[i].markdown(f"**{m}**: <span style='color:{cor_a}'>{utils.to_br_currency(liq_a)}</span>", unsafe_allow_html=True)
