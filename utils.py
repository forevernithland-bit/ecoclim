...
# ==========================================
# GERAÇÃO DE PDF (v.2.1 - Layout Corrigido)
# ==========================================
def gerar_pdf_orcamento(nome, tel, capa_tipo, df_items, d_serv, v_serv, d_out, v_out, total, obs, mostrar_un):
    ...
    for _, row in df_items.iterrows():
        if row['Quantidade'] > 0:
            ...
            # Busca a descrição e calcula o espaço dinamicamente
            ...
            if desc and desc != "nan":
                ...
                for linha in linhas_desc:
                    if linha.strip():
                        # Adiciona um prefixo se não for garantia/quantidade
                        prefix = "Detalhes: " if linha == linhas_desc[0] and not (linha.startswith("QUANTIDADE") or linha.startswith("GARANTIA")) else ""
    ...
    # Aumentado o espaço matemático para evitar que a descrição de cima atropele esta barra
    y -= 0.6*cm 
    p.setFillColor(colors.HexColor("#004488"))
    p.rect(2*cm, y, largura - 4*cm, 0.7*cm, fill=1, stroke=0)
    p.setFillColor(colors.white)
    ...
    p.rect(2*cm, y - 0.2*cm, largura - 4*cm, 1.2*cm, fill=1, stroke=0)
    p.setFillColor(colors.black)
    p.setFont("Helvetica-Bold", 12)
    p.drawString(2.3*cm, y + 0.3*cm, "INVESTIMENTO TOTAL")
    # Garantido que está a usar o 'total' que é VENDA pura!
