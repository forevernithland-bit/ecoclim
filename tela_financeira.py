...
def renderizar():
    ...
    with st.container(border=True):
        ...
        tel_c = c2.text_input("WhatsApp", placeholder="(31) 99999-9999", key="tel_cliente_orc")
    ...
    with st.container(border=True):
        ...
        for i in range(len(df_ed)):
            ...
            # Se o usuário escolheu um produto diferente do que estava antes
            if p != p_prev and p in lista_p:
                match = cat_p[cat_p['Item'] == p]
                if not match.empty:
                    df_ed.at[i, 'Venda (R$)'] = float(match['Venda (R$)'].values[0])
                    
                    # Puxa a descrição da base com segurança
                    ...
                    # Quando escolhe o produto, seta pra 1 pra ajudar
