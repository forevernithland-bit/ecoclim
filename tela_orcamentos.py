            ...
            if nome_c:
                ...
                try:
                    ...
                    st.session_state.supabase.table("servicos_andamento").insert({
                        "numero_orcamento": num_orc, "nome_cliente": nome_c, "telefone_cliente": tel_f, 
                        "produtos_adquiridos": ", ".join([f"{int(r['Quantidade'])}x {r['Produto da Base']}" for _, r in df_ed.iterrows() if r['Quantidade']>0]),
                        "servicos_adquiridos": d_s, "valor_venda_total": total_geral, "status_projeto": "Orçamento Enviado", "detalhamento_itens": snapshot
                    }).execute()
                    st.session_state['pdf_oficial'] = utils.gerar_pdf_orcamento(nome_c, tel_f, capa, df_ed, d_s, v_s, d_o, v_o, total_geral, obs, mostrar_pdf)
                    st.session_state['oficial_filename'] = f"{num_orc}_{nome_c}.pdf"
                    st.success(f"✅ Orçamento {num_orc} salvo!")
