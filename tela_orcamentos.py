...
def renderizar():
    ...
    with st.container(border=True):
        ...
        for i in range(len(df_ed)):
            ...
            if p in lista_p and df_ed.at[i, 'Venda (R$)'] == 0:
                ...
                if not match.empty:
                    # ====================================================================
                    # CORREÇÃO CRÍTICA (LINHA 113): MUDADO DE 'Custo (R$)' PARA 'Venda (R$)'
                    # ====================================================================
                    df_ed.at[i, 'Venda (R$)'] = float(match['Venda (R$)'].values[0])
                    
                    if df_ed.at[i, 'Descrição'] == "":
    ...
    with col_prev:
        st.info("👁️ PASSOS DE CONFERÊNCIA: \nGere a prévia do PDF, abra, confira se todos os valores e as descrições dos equipamentos estão alinhados, e se o Investimento Total está correto. Somente após isso Salve Oficialmente.")
    ...
    with col_salvar:
        st.warning("⚠️ SALVAMENTO OFICIAL: \nIsso registrará o orçamento com um número único, congelará os custos e as descrições atuais e liberará para faturamento. CERTIFIQUE-SE DE TER CONFERIDO A PRÉVIA ACIMA.")
            ...
            if not nome_c:
                ...
                for _, row in df_ed.iterrows():
                    ...
                    if nome_p in lista_p:
                        # Busca custo congelado para auditoria
