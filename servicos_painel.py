...
def exibir_painel_detalhado(projeto_selecionado, supabase, df_taxas_config, df_produtos, prefix_key, lista_instaladores):
    ...
    # Motor cravado pelo índice (Evita que o Streamlit quebre se você apagar linhas)
    ...
    if novo_status not in ["Orçamento Enviado", "Orçamento Cancelado", "Excluir"]:
        ...
        with st.container(border=True):
            ...
            # Controle rigoso do uploader: Muda a chave a cada upload para esvaziar a memória e não dar loop!
                    ...
                    with st.spinner("Salvando no Drive e no ERP..."):
                        ...
                        if sucesso:
                            ...
                            try:
                                ...
                                st.session_state[upload_key] += 1 # Ejeta o PDF da tela!
                                st.rerun() 
