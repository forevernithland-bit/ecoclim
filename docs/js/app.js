...
async function viewAgenda() {
  ...
  badgeAgenda = 0;
  const [visitas, pendentesNovas, instaladoresDisponiveis] = await Promise.all([
    puxarVisitas(sessao.instaladorVinculado),
    visitasPendentesNovas(),
    sessao.admin ? puxarInstaladores() : Promise.resolve([]),
  ]);
    ...
    <div class="cartao">
      ...
      ${!pendente && sessao.admin ? `
        ...
        <div id="edicao-visita-${v.id}" style="display:none; margin-top:10px;">
          <label>Instalador${!v.instalador ? " ⚠️ ainda sem responsável" : ""}</label>
          <select id="ev-instalador-${v.id}">
            <option value="">-- sem instalador --</option>
            ${instaladoresDisponiveis.map((nome) => `<option value="${escapeAttr(nome)}" ${nome === v.instalador ? "selected" : ""}>${escapeHTML(nome)}</option>`).join("")}
          </select>
          <label>Cliente</label>
  ...
  function ligarAcoesVisita() {
    ...
    raiz.querySelectorAll("[data-salvar-edicao-visita]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        try {
          const visitaAtual = visitas.find((x) => x.id === Number(vid));
          const novoInstalador = document.getElementById(`ev-instalador-${vid}`).value || null;
          const clienteEditado = document.getElementById(`ev-cliente-${vid}`).value.trim();
          await atualizarVisitaAdmin(Number(vid), {
            instalador: novoInstalador,
            cliente_nome: clienteEditado,
          ...
          // Avisa o instalador só quando o responsável muda de verdade (foi
          // atribuído agora ou trocado) — reeditar cliente/data sem trocar
          // o instalador não deve gerar notificação de novo.
          if (novoInstalador && novoInstalador !== visitaAtual?.instalador) {
            notificar({
              titulo: "🗓️ Você foi atribuído a uma visita",
              mensagem: `${clienteEditado || "Cliente"} — o Breno te colocou como responsável.`,
              instalador: novoInstalador,
              tag: `agenda-${vid}`,
            });
          }
